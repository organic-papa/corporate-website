<div class="mainLeft">
	<a href="https://organic-papa.sakura.ne.jp/post/mail/"><img src="../images/ban_ec.jpg" width="225" height="208"></a>
	<ul class="lNav">
		<li class="lNav01"><a href="/farm">農園づくり</a></li>
		<ul>
			<li class="{{{ $active == 'farmProduct' ? 'active' : ''}}}"><a href="/farm/product">農園づくりのコンセプト</a></li>
			<li class="{{{ $active == 'farmOrderPlantation' ? 'active' : ''}}}"><a href="/farm/order_plantation">注文農園のススメ</a></li>
			<li class="{{{ $active == 'farmNewFarmers' ? 'active' : ''}}}"><a href="/farm/newfarmers">新規就農をご検討の方へ</a></li>
		</ul>
	</ul>
	<a href="/post/recipes"><img src="/images/ban_re.jpg" width="225" height="208"></a>
	<div class="leftBanner">
		<h3>「家庭の食育」発売中！</h3>
		<a href="/post/book"><img src="/images/kateino_shokuiku.jpg" width="350"></a>
		<p>服部幸應さん監修「家庭の食育」を発売しました。</p>
		<p class="rLink"><a href="/post/book">＞続きを読む</a></p>
	</div>
	<div class="infoBanner">
		<h3>オーガニックパパへの<br />お問合わせ先</h3>
		<div class="infoBannerIn">
			<p class="infoBannerTel">092-737-7311</p>
			<p>営業時間10:00～19:00</p>
		</div>
	</div>
</div>
